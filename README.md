<h1 align="center">Hi there, I'm John Jerome Bernal 👋</h1>
<h3 align="center">Software Engineer | Web Developer</h3>

<p align="center">
  <a href="https://portfolio-v3-juv9.vercel.app/" target="_blank"><img src="https://img.shields.io/badge/Portfolio-jer--portfolio-181717?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="mailto:johnjeromebernal@gmail.com"><img src="https://img.shields.io/badge/Email-johnjeromebernal%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
</p>

---

### 🚀 About Me

- 🎓 Software Engineer with **5+ years** of experience building, maintaining, and deploying web applications
- 💼 Currently a **Software Engineer at YNS Philippines Inc.**, leading sprint execution and owning multiple production projects
- 🛠️ Comfortable across the full stack — from requirements analysis and technical specs to production deployment and server maintenance
- 🌐 Domain experience in **e-commerce, restaurant reservations, online pharmacy, business & labor matching, payments, logistics, and dispatch management**
- 📍 Based in Philippines
- 📫 Reach me at **johnjeromebernal@gmail.com**

---

### 🧰 Tech Stack

**Languages**
<p>
  <img src="https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/Dart-0175C2?style=flat-square&logo=dart&logoColor=white" />
</p>

**Frameworks & Libraries**
<p>
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/CakePHP-D33C43?style=flat-square&logo=cakephp&logoColor=white" />
  <img src="https://img.shields.io/badge/Symfony-000000?style=flat-square&logo=symfony&logoColor=white" />
  <img src="https://img.shields.io/badge/Vue.js-4FC08D?style=flat-square&logo=vue.js&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white" />
  <img src="https://img.shields.io/badge/jQuery-0769AD?style=flat-square&logo=jquery&logoColor=white" />
  <img src="https://img.shields.io/badge/Inertia.js-9553E9?style=flat-square" />
  <img src="https://img.shields.io/badge/Bootstrap-7952B3?style=flat-square&logo=bootstrap&logoColor=white" />
  <img src="https://img.shields.io/badge/PHPUnit-3776AB?style=flat-square" />
</p>

**Databases & Cloud**
<p>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white" />
</p>

**Tools & Platforms**
<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/Bitbucket-0052CC?style=flat-square&logo=bitbucket&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white" />
</p>

**Integrations**
<p>
  <img src="https://img.shields.io/badge/Stripe%20API-635BFF?style=flat-square&logo=stripe&logoColor=white" />
  <img src="https://img.shields.io/badge/PaymentLite-4B4B4B?style=flat-square" />
  <img src="https://img.shields.io/badge/Epsilon-4B4B4B?style=flat-square" />
</p>

---

### 💼 Experience Highlights

**Software Engineer** — YNS Philippines Inc. *(Jul 2022 – Present)*
- Lead sprint execution: backlog creation, task breakdown, story point estimation, technical specs, and delegation
- Own delivery across multiple projects — from requirements analysis to production deployment and server maintenance
- Lead developer on a labor & material matching platform (job postings, payments, material sharing, community bulletin)
- Independent developer for an online pharmacy platform
- One of two primary developers on an ECCUBE-based e-commerce and restaurant reservation system

**Junior Software Engineer** — YNS Philippines Inc. *(Dec 2021 – Jul 2022)*
- Built and tested features from requirements, business-flow diagrams, and system documentation
- Implemented PHPUnit test suites and authored test cases from acceptance requirements
- Delivered features for a business matching platform, an e-commerce PDF-processing module, and a garbage collection/truck dispatch management system

**Associate Software Engineer** — YNS Philippines Inc. *(Jul 2021 – Dec 2021)*
- Delivered a microblog application as part of software engineering training
- Resolved defects and maintained existing applications
- Contributed to upgrading a legacy CakePHP application to its latest supported version

📄 [View full resume / portfolio](https://portfolio-v3-juv9.vercel.app/)

---

<p align="center">
  <i>Thanks for stopping by — let's connect and build something great!</i>
</p>